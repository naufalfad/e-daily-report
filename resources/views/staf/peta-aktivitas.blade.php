...
<style>
    ...
    .map-container {
        height: min(60vh, 500px); /* maksimal 500px, minimal proporsional */
        width: 100%;
        position: relative;
    }
    ...
    Alpine.data('mapPageData', () => ({
        ...
        initMap() {
            this.$nextTick(() => { // pastikan DOM siap
                this.map = L.map('map', { zoomControl: true }).setView([-4.557, 136.885], 13);
                ...
                const baseLayers = { "Google Maps": googleRoadmap, "Google Satelit": googleSatelit };
        ...
        initDatePickers() {
            this.$nextTick(() => {
                ['tgl_dari','tgl_sampai'].forEach(id => {
                    ...
                    if(input && btn) btn.addEventListener('click',()=>{ try{input.showPicker()}catch(e){input.focus()} });
                });
            });
        }
    }));
