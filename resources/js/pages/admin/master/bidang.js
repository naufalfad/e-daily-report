...
document.addEventListener('DOMContentLoaded', () => {
    // Config
    const API_URL = '/api/admin/master/bidang';
    ...
    // [CUSTOM] Render Table Khusus Bidang
        ...
        data.forEach((item, index) => {
            ...
            const html = `
                <tr class="hover:bg-slate-50 transition-colors border-b border-slate-100">
                    ...
                    <td class="px-6 py-4 font-semibold text-slate-700">${item.nama_bidang}</td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700 border border-emerald-100">
                            <i class="fas fa-users text-[10px]"></i> ${item.users_count || 0} Personil
                        </span>
                    </td>
                    <td class="px-6 py-4 text-center">
    ...
    // ... (Fungsi renderPagination & showLoading SAMA - Copy Paste) ...
    ...
    window.openModal = function(type, data = null) {
        document.getElementById('formBidang').reset();
        if (type === 'add') {
    ...
    window.closeModal = function() {
        ...
        setTimeout(() => { document.getElementById('modalBidang').addClass('hidden'); }, 200);
    };

    window.deleteData = function(id) {
        Swal.fire({
            title: 'Hapus Bidang?',
            text: "Data tidak bisa dikembalikan. Pastikan bidang kosong.",
            icon: 'warning',
