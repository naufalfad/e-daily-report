        ...
        <form id="form-lkh">
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-wrap items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" type="button" onclick="submitForm('waiting_review')" class="rounded-[10px] bg-[#0E7A4A] px-4 py-2 text-sm font-normal text-white
                            hover:bg-[#0b633b]">
                        Kirim LKH
                    </button>
                </div>
...
<scr ipt>
    ...
    docu ment.addEventListener("DOMContentLoaded", async function() {
    ...
    if (toke n) headers["Authorization"] = "Bearer " + token;
    ...
    // 🔥 RENDER AKTIVITAS
    // ===== =======================================================
    function ren derAktivitas(aktivitas) {
    cons t listContainer = document.getElementById("aktivitas-list");
    listContainer.innerHTML = "";

    const iconPaths = {
    pending: "{{ asset('assets/icon/pending.svg') }}",
    approve: "{{ asset('assets/icon/approve.svg') }}",
    reje ct: "{{ asset('assets/icon/reject.svg') }}"
    };
    ...
    akti vitas.forEach(item => {
    cons t d = new Date(item.tanggal_laporan);
    cons t tanggal = d.toLocaleDateString("id-ID", {
    day: "2-digit",
    month: "short",
    year: "numeric"
    });
    ...
    waktu_simpan: `Disimpan: ${d.toLocaleDateString('id-ID', {day:'numeric', month:'long'})} |
    ...
    // ============================================================
    async functi on submitForm(statusType) {
    if (even t) event.preventDefault();

    cons t token = localStorage.getItem("auth_token");
    const form = document.getElementById("form-lkh");
    cons t formData = new FormData(form);

    form Data.set("status", statusType);

    // Validasi — Waiting Review
    if (statusTy pe === "waiting_review") {
    const re quired = [
    ...
    cons t response = await fetch(url, {
    meth od: "POST",
    head ers: {
    "Authorization": `Bearer ${token}`,
    "Accept": "application/json"
