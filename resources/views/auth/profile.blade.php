...
<div class="container mx-auto px-4 py-6">
    ...
    @if(session('success'))
        <div class="mb-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-xl relative shadow-sm" role="alert">
            <div class="flex items-center">
                <i class="fas fa-check-circle mr-2 text-xl"></i>
                <span class="font-bold mr-1">Sukses!</span> {{ session('success') }}
            </div>
        </div>
    @endif

    @if(session('warning'))
        <div class="mb-4 bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-xl relative shadow-sm" role="alert">
            <div class="flex items-center">
                <i class="fas fa-exclamation-triangle mr-2 text-xl"></i>
                <span class="font-bold mr-1">Perhatian!</span> {!! session('warning') !!}
            </div>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-[20px] shadow-[0_4px_20px_rgba(0,0,0,0.05)] overflow-hidden border border-gray-100">
                <div class="bg-[#1C7C54] px-6 py-4 flex justify-between items-center">
                    <h6 class="font-bold text-white text-lg">Data Kepegawaian</h6>
                    <span class="bg-white/20 text-white text-xs px-2 py-1 rounded-full uppercase tracking-wide border border-white/20">
                ...
                <div class="p-6 text-center">
                    ...
                    <p class="text-gray-500 text-sm mb-4 font-mono bg-gray-100 inline-block px-2 py-1 rounded">{{ $user->nip ?? '-' }}</p>
                    
                    <div class="border-t border-gray-100 my-4"></div>

                    <div class="text-left space-y-4">
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Jabatan</label>
                            <div class="text-gray-800 font-medium bg-gray-50 px-3 py-2 rounded-lg border border-gray-100 flex items-center">
                        ...
                        <div>
                            <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Unit Kerja</label>
                            <div class="text-gray-800 text-sm bg-gray-50 px-3 py-2 rounded-lg border border-gray-100 flex items-center">
                        ...
                        <div class="bg-blue-50 p-4 rounded-xl border border-blue-100 relative overflow-hidden">
                            <i class="fas fa-user-tie absolute -right-3 -bottom-3 text-6xl text-blue-100 opacity-50 transform rotate-12"></i>
                            
                            <label class="flex items-center text-xs font-bold text-blue-600 uppercase tracking-wider mb-2 relative z-10">
                            ...
                            <small class="text-gray-500 text-xs relative z-10">{{ $user->atasan->jabatan->nama_jabatan ?? '' }}</small>
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-[20px] shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-gray-100 overflow-hidden min-h-[500px]">
                ...
                <div class="flex border-b border-gray-200" id="tabs-nav">
                    ...
                    <button onclick="switchTab('account')" id="tab-btn-account"
                        ...
                        @if($hasAccountErrors)
                            <span class="ml-2 bg-red-100 text-red-600 text-xs px-2 py-0.5 rounded-full font-bold animate-pulse">!</span>
                        @endif
                    </button>
                </div>
                        ...
                        <form action="{{ route('profil.update-biodata') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-6">
                                ...
                                <div class="flex items-center justify-center w-full">
                                    <label for="foto_profil" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 transition group">
                                        <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                            <i class="fas fa-cloud-upload-alt text-3xl text-gray-400 mb-2 group-hover:text-[#1C7C54] transition"></i>
                                            <p class="text-sm text-gray-500 group-hover:text-gray-700"><span class="font-semibold">Klik untuk upload</span></p>
                                            <p class="text-xs text-gray-400 mt-1">JPG, PNG, WEBP (Max 2MB)</p>
                                        </div>
                                        <input id="foto_profil" name="foto_profil" type="file" class="hidden" accept="image/*" />
                                    </label>
                                </div>
                                <div id="file-name-display" class="mt-2 text-sm text-[#1C7C54] font-medium hidden flex items-center">
                            ...
                            <div class="text-right">
                                <button type="submit" class="bg-[#1C7C54] hover:bg-[#156343] text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-green-900/10 transition-all transform hover:-translate-y-1 flex items-center justify-center ml-auto">
                    ...
                    <div id="tab-content-account" class="{{ $activeTab == 'account' ? 'block' : 'hidden' }}">
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-orange-700">
                                        <strong>Penting:</strong> Mengubah Username atau Password akan mempengaruhi cara Anda login.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <form action="{{ route('profil.update-account') }}" method="POST" id="form-account">
                            ...
                            <div class="border-t border-gray-100 my-8 relative">
                                <span class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white px-2 text-gray-400 text-sm">Keamanan</span>
                            </div>
...
<div id="tailwind-modal" class="hidden fixed inset-0 z-[999] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity backdrop-blur-sm"></div>

    <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border-t-4 border-red-500">
            <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                        <h3 class="text-lg font-bold leading-6 text-gray-900" id="modal-title">Konfirmasi Perubahan Login</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">
                                Anda sedang melakukan perubahan pada <strong>Akses Login (Username/Password)</strong>.
                                <br><br>
                                Mohon pastikan Anda mengingat data baru ini agar tidak terkunci dari sistem pada login berikutnya.
                            </p>
                        </div>
                    </div>
                </div>
