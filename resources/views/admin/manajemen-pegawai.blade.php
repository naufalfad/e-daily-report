...
.form-input-tegas {
    @apply w-full rounded-lg border-2 border-slate-400 bg-white px-4 py-2.5 text-sm font-bold text-slate-800 focus: border-[#1C7C54] focus:ring-2 focus:ring-[#1C7C77]/20 transition-all;
}
...
<div x-data="manajemenPegawaiData()" x-init="initPage()" class="flex-1 flex flex-col min-h-0 relative">
    ...
    <section class="flex-1 flex flex-col rounded-2xl bg-white ring-1 ring-slate-200 px-6 py-5 mb-0">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
            ...
            <div class="flex gap-3">
                ...
                {{-- [MODIFIKASI: TAMBAH BUTTON DOWNLOAD] --}}
                <a href="{{ asset('assets/template/template_import_user.csv') }}" download
                    class="bg-[#2C8E4D] text-white px-4 py-2 rounded-[10px] text-sm font-medium hover:brightness-95 flex items-center gap-2">
                ...
                <button @click="toggleUpload(true)"
                    class="bg-[#128C60] text-white px-4 py-2 rounded-[10px] text-sm font-medium hover:brightness-95 flex items-center gap-2">
        ...
        <div class="flex gap-3 mb-4">
            <div class="w-full md:w-1/3">
                <input type="text" x-model="search" @input.debounce.500ms="fetchData()" placeholder="Cari Nama / NIP..."
                    class="w-full rounded-[10px] border-2 border-slate-300 bg-slate-50 px-4 py-2.5 text-sm focus:border-[#1C7C54] font-bold">
        ...
        <div class="flex-1 overflow-x-auto border border-slate-200 rounded-xl relative">
            <div x-show="isLoading" class="absolute inset-0 z-10 bg-white/80 flex items-center justify-center"><span
                    class="font-bold text-slate-600 animate-pulse">Memuat data...</span></div>
                    ...
                    <template x-for="item in items" :key="item.id">
                        <tr class="hover:bg-slate-50">
                            <td class="py-3 px-4">
                                ...
                                <div class="text-xs font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded mt-1 w-fit"
                                    x-text="item.nip"></div>
                            </td>
                            <td class="py-3 px-4 font-mono font-bold text-blue-600" x-text="item.username"></td>
                            <td class="py-3 px-4 font-medium text-slate-600" x-text="item.jabatan?.nama_jabatan || '-'">
                            </td>
                            <td class="py-3 px-4 font-medium text-slate-600" x-text="item.unit_kerja?.nama_unit || '-'">
                            </td>
                            <td class="text-center py-3 px-4 flex justify-center gap-2">
                                <button @click="openModalEdit(item)"
                                    class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg"><svg
    ...
    <div x-show="openAdd || openEdit" x-cloak
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto border border-slate-200"
            ...
            <div
                class="px-8 py-5 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white z-20">
                <h3 class="text-xl font-bold text-slate-800"
                    x-text="openEdit ? 'Edit Pegawai (Data HR)' : 'Tambah Pegawai (Data HR)'"></h3>
            ...
            <div class="px-8 py-8">
                <form @submit.prevent="submitForm(openEdit ? 'edit' : 'add')" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {{-- DATA DIRI (HR) --}}
                        <div><label class="form-label-tegas">Nama Lengkap</label><input type="text"
                                x-model="formData.name" class="form-input-tegas" placeholder="Nama Pegawai"></div>
                        <div><label class="form-label-tegas">NIP</label><input type="text" x-model="formData.nip"
                                class="form-input-tegas" placeholder="NIP"></div>
                        ...
                        <div>
                            ...
                            <select x-model="formData.bidang_id" class="form-input-tegas cursor-pointer"
                                :disabled="!formData.unit_kerja_id">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select x-model="formData.atasan_id"
                                    class="form-input-tegas cursor-pointer bg-emerald-50 border-emerald-400"
                                ...
                                <div x-show="isFetchingAtasan" class="absolute right-3 top-3"><svg
                                        class="animate-spin h-5 w-5 text-emerald-600" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <p class="text-xs text-emerald-600 mt-1 font-medium" x-show="atasanList.length > 0">Sistem
                                merekomendasikan atasan sesuai struktur.</p>
                        </div>
                        ...
                        <div x-show="openAdd"
                            class="col-span-2 p-3 rounded-lg bg-yellow-50 border-l-4 border-yellow-500 text-sm text-yellow-800 font-medium">
                        ...
                        <div x-show="openEdit"
                            class="col-span-2 p-3 rounded-lg bg-blue-50 border-l-4 border-blue-500 text-sm text-blue-800 font-medium">
                            <p class="font-bold">Informasi Kredensial Akun:</p>
                            <p>Untuk mengubah Username, Password, atau Hak Akses (Role), silakan gunakan menu **Akun
                                Pengguna**.</p>
                        </div>
                    ...
                    <div class="mt-8 pt-4 border-t border-slate-100 flex justify-end gap-4">
                        <button type="button" @click="openAdd ? toggleAdd(false) : toggleEdit(false)"
                            class="px-6 py-2.5 rounded-lg border-2 border-slate-300 text-slate-700 font-bold hover:bg-slate-100 transition">Batal</button>
                        <button type="submit"
                            class="px-6 py-2.5 rounded-lg bg-[#128C60] text-white font-bold hover:bg-emerald-700 hover:shadow-lg transition transform hover:-translate-y-0.5">Simpan
                            Data Pegawai</button>
                    </div>
                </form>
            </div>
    ...
            <div x-show="openUpload" x-cloak
        class="fixed inset-0 z-[65] flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm"
        x-transition.opacity>
                    <div
            class="relative w-full max-w-[520px] bg-white rounded-[15px] shadow-xl px-6 md:px-8 py-6 md:py-7"
            @click.away="toggleUpload(false)">
                            <button @click="toggleUpload(false)"
                class="absolute right-6 top-5 text-slate-400 hover:text-slate-600 text-xl">&times;</button>
            ...
            <form @submit.prevent="submitImport()" class="space-y-5">
                ...
                <input type="file" x-ref="csvFile" @change="fileUpload = $event.target.files[0]"
                    accept=".xlsx, .xls, .csv" hidden>

                <div @click="$refs.csvFile.click()" class="w-full rounded-[20px] border-2 border-dashed transition-colors 
                        px-6 py-10 flex flex-col items-center justify-center text-center cursor-pointer 
                        "
                    :class="fileUpload ? 'border-[#1C7C54] bg-emerald-50' : 'border-slate-300 bg-slate-50/60 hover:border-[#1C7C54]'">

                    <img src="{{ asset('assets/icon/upload-excel.svg') }}" class="h-10 w-10 mb-3"
                        :class="fileUpload ? 'opacity-100' : 'opacity-70'">

                    <p x-show="!fileUpload" class="text-sm text-slate-500 font-bold">Pilih File Excel (.xls, .xlsx) atau
                        CSV</p>
                    <p x-show="fileUpload" class="text-sm text-[#1C7C54] font-bold"
                        x-text="'File terpilih: ' + fileUpload.name"></p>
                    <p class="text-xs text-slate-500 mt-1" x-show="!fileUpload">Klik di sini untuk memilih file</p>
                </div>

                <div class="mt-6 flex justify-end gap-4">
                    <button type="button" @click="toggleUpload(false)"
                        class="px-6 py-2 rounded-lg bg-[#B6241C] text-white font-bold hover:brightness-95">Batalkan</button>
                    <button type="submit"
                        class="px-6 py-2 rounded-lg bg-[#0E7A4A] text-white font-bold hover:brightness-95"
