    ...
    public function run()
    {
        ...
        try {
            ...
            foreach ($dataWilayah as $distrik) {
                // FIXED: Mencari Kecamatan (Case Insensitive untuk Postgres)
                $kecamatan = MasterKecamatan::where('kabupaten_id', $kabupatenId)
                    ->where(function($query) use ($distrik) {
                        $query->whereRaw('UPPER(nama) = ?', [strtoupper($distrik['nama_distrik'])])
                              ->orWhere('nama', 'LIKE', '%' . $distrik['nama_distrik'] . '%');
                    })
                    ->first();

                if (!$kecamatan) {
                    $this->command->warn("SKIP: Kecamatan '{$distrik['nama_distrik']}' tidak ditemukan di database.");
                    continue;
                }

                foreach ($distrik['daftar_desa'] as $desa) {
                    // FIXED: Mencari Kelurahan (Case Insensitive untuk Postgres)
                    // Data CSV Anda menggunakan HURUF KAPITAL (KOPERAPOKA), JSON Anda Title Case (Koperapoka).
                    // Kita paksa pencarian menggunakan UPPER() agar cocok.
                    
                    $kelurahan = MasterKelurahan::where('kecamatan_id', $kecamatan->id)
                        ->where(function($query) use ($desa) {
                            $query->whereRaw('UPPER(nama) = ?', [strtoupper($desa['nama_desa'])])
                                  ->orWhere('nama', 'LIKE', '%' . $desa['nama_desa'] . '%');
                        })
                        ->first();
                    ...
                    if ($kelurahan) {
                        ...
                        // Debugging tambahan: Tampilkan apa yang dicari vs kemungkinan error
                        $this->command->warn("SKIP: Desa '{$desa['nama_desa']}' (Kec. {$distrik['nama_distrik']}) tidak ditemukan.");
                    }
                }
            }
            
            DB::commit();
            $this->command->info("BERHASIL: {$updatedCount} Desa/Kelurahan di Mimika berhasil di-update koordinatnya.");

        } catch (\Exception $e) {
            DB::rollBack();
            $this->command->error("ERROR: Terjadi kesalahan saat seeding. Rollback dilakukan.");
