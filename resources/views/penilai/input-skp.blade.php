...
<section x-data="skpPageData()" x-init="initPage()"
    ...
    <div x-show="openDetail" style="display: none;" 
        ...
        <div class="relative w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl" @click.outside="openDetail = false">
            <button @click="openDetail = false" class="absolute top-4 right-5 text-slate-400 hover:text-slate-600">&times;</button>
            ...
            <template x-if="detailData">
                <div class="mt-6 space-y-4 text-sm">
                    <div class="grid grid-cols-2 gap-4 border-b border-slate-100 pb-4">
                        <div><label class="text-xs text-slate-500">Mulai</label><p class="font-medium" x-text="formatDate(detailData.periode_mulai)"></p></div>
                        <div><label class="text-xs text-slate-500">Selesai</label><p class="font-medium" x-text="formatDate(detailData.periode_selesai)"></p></div>
                    </div>
                    <div><label class="text-xs text-slate-500">Sasaran</label><p x-text="detailData.nama_skp"></p></div>
                    <div><label class="text-xs text-slate-500">Indikator</label><p x-text="detailData.indikator"></p></div>
                    <div><label class="text-xs text-slate-500">Target</label><p class="font-bold" x-text="detailData.target"></p></div>
                    
                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-200">
                        <button @click="openDetail = false" class="px-4 py-2 bg-slate-100 rounded text-slate-700">Tutup</button>
                        <button @click="openEditModal()" class="px-4 py-2 bg-[#155FA6] text-white rounded">Edit SKP</button>
                    </div>
                </div>
            </template>
        </div>
                ...
                <template x-if="editData">
                    <div class="space-y-4 text-sm">
                        <div class="grid grid-cols-2 gap-4">
                            <div><label class="block text-xs font-semibold mb-1">Mulai</label><input type="date" x-model="editData.periode_mulai" class="w-full border rounded p-2"></div>
                            <div><label class="block text-xs font-semibold mb-1">Selesai</label><input type="date" x-model="editData.periode_selesai" class="w-full border rounded p-2"></div>
                        </div>
                        <div><label class="block text-xs font-semibold mb-1">Sasaran</label><input type="text" x-model="editData.nama_skp" class="w-full border rounded p-2"></div>
                        <div><label class="block text-xs font-semibold mb-1">Indikator</label><input type="text" x-model="editData.indikator" class="w-full border rounded p-2"></div>
                        <div><label class="block text-xs font-semibold mb-1">Rencana Aksi</label><textarea x-model="editData.rencana_aksi" class="w-full border rounded p-2" rows="2"></textarea></div>
                        <div><label class="block text-xs font-semibold mb-1">Target</label><input type="number" x-model="editData.target" class="w-full border rounded p-2"></div>
                    </div>
                </template>
                <div class="pt-4 border-t flex justify-end gap-3">
                    <button type="button" @click="openEdit = false" class="px-4 py-2 border rounded">Batal</button>
                    <button type="submit" class="px-4 py-2 bg-[#0F4C75] text-white rounded" :disabled="isLoading">Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('skpPageData', () => ({
        ...
        formatDate(date) {
            if(!date) return '-';
            try { return new Date(date).toLocaleDateString('id-ID', {day:'2-digit', month:'short', year:'numeric'}); }
            catch(e) { return date; }
        }
    }));
});
